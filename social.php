...
add_action( 'admin_notices', 'simplesocial_notice' );
function simplesocial_notice() {
	$user_id = get_current_user_id();
	$admin_url = ( isset( $_SERVER['HTTPS'] ) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http' ) . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
	$param = ( count( $_GET ) ) ? '&' : '?';
	if ( !get_user_meta( $user_id, 'simplesocial_notice_dismissed_4' ) && current_user_can( 'manage_options' ) ) {
		echo '<div class="notice notice-info"><p><a href="' . esc_url( $admin_url ), esc_html( $param ) . 'dismiss" class="alignright" style="text-decoration:none"><big>' . esc_html__( 'Ⓧ', 'simplesocial' ) . '</big></a>' . wp_kses_post( __( '<big><strong>💛 Thank you for using SimpleSocial!</strong></big>', 'simplesocial' ) ) . '<br><br><a href="https://wordpress.org/support/plugin/simplesocial/reviews/#new-post" class="button-primary" style="background-color:purple;border-color:purple" target="_blank">' . esc_html__( 'Review', 'simplesocial' ) . '</a> <a href="mailto:bhadaway@gmail.com" class="button-primary" style="background-color:orange;border-color:orange" target="_blank">' . esc_html__( 'Support', 'simplesocial' ) . '</a> <a href="https://simplesocial.pro/" class="button-primary" style="background-color:green;border-color:green" target="_blank">' . esc_html__( 'Upgrade', 'simplesocial' ) . '</a></p></div>';
	}
}
...
function simplesocial_notice_dismissed() {
	...
	if ( isset( $_GET['dismiss'] ) ) {
		add_user_meta( $user_id, 'simplesocial_notice_dismissed_4', 'true', true );
	}
}
...
function simplesocial_options_page() {
	...
	<div id="simplesocial" class="wrap">
        ...
        <p><?php _e( 'Create a custom menu under <em>Appearance > Menus</em>, add custom links to it, add the desired platform name to each link\'s respective Navigation Label (Facebook, Twitter, etc.), assign the menu to the SimpleSocial location, and then use one of the following to add icons anywhere in your theme:', 'simplesocial' ); ?></p>
	...
	if ( ( $locations = get_nav_menu_locations() ) && isset( $locations[ $menu_name ] ) ) {
		...
		foreach ( ( array ) $menu_items as $key => $menu_item ) {
			...
			if ( strpos( strtolower( $title ), 'facebook' ) !== false || strpos( $url, 'facebook' ) !== false ) {
				$social = 'facebook';
			} elseif ( strpos( strtolower( $title ), 'x' ) !== false || strpos( $url, 'x' ) !== false ) {
				$social = 'x';
				...
				$social = 'instagram';
			} elseif ( strpos( strtolower( $title ), 'tiktok' ) !== false || strpos( $url, 'tiktok' ) !== false ) {
				$social = 'tiktok';
